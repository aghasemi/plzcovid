...
chosen_plzs = st.multiselect("",plzs,['8004','8400'])
...
dates = st.date_input("",[start_date,end_date],start_date,end_date)
if len(dates)<2:
    start_date = dates[0]
else:
...
for i,plz in enumerate(chosen_plzs):
    ...
    chart  += alt.Chart(cases_per_plz).mark_area(opacity = 0.25,color = color).encode(x=alt.X('Date', axis=alt.Axis(labels=False)), y=alt.Y(plot_y_axis_column_min,title=plot_y_axis_title), y2=alt.Y2(plot_y_axis_column_max), tooltip = tooltip)
    chart  += alt.Chart(cases_per_plz).mark_line(color = color).encode(x=alt.X('Date', axis=alt.Axis(labels=True)), y=alt.Y(plot_y_axis_column_avg,title=plot_y_axis_title), tooltip = tooltip)
